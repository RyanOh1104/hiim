...
def snake_to_camel_case(s):
    ...
    # new_string = ''.join([word.what() for word in new_string.split('_')])
...
# 이 아래 함수의 결과가 all_events 들어갔을 때 dictionary 형태로 보여지는 함수
def events_to_json(events_queryset):
    ...
    # 아래는 내가
    events_values = list(events_queryset.values('id', 'what', 'when', 'end', 'all_day'))  # all_day는 default로!
